...
const usePostService = () => {
  ...
  async function getDayPosts(colorDay, page = 0, limit = 10) {
  ...
  async function setLikePost(postId, newState = true) {
    const method = newState ? "POST" : "DELETE";
    const response = await requestWithTokenRetry(
      `${config.serverUrl}/posts/${postId}/likes`,
      {
        method,
        credentials: "include",
      },
  ...
  async function setReportPost(postId, reportDto) {
    const response = await requestWithTokenRetry(
      `${config.serverUrl}/posts/${postId}/reports`,
      {
        method: "POST",
        credentials: "include",
